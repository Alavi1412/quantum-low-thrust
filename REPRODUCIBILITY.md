...
- Tail-coast fixed-final-time hard-catalog recovery diagnostic
  ...
  `no_recovery_variables`; the separate all-configured two-segment row
  selects/evaluates all 13 configured two-segment masks, optimizes
  recovery-variable branches with accepted fallbacks `constant_y_minus_0p5`,
  `constant_x_minus_0p5`, and `constant_x_plus_0p5` on masks 0, 6, and 7,
  and treats mask 12 as threshold-feasible direct evaluation because it has
  `no_recovery_variables`):
