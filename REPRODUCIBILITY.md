...
`data/results/online_quantum_search/*`,
`data/results/online_branch_refinement/*`,
`data/results/structured_grover_one_coast/*`,
`data/results/structured_grover_k10_full/*`,
...
| Claim evidence ledger | `py -3.11 scripts\run_claim_evidence_ledger.py` | Recorded summary/statistical CSV/JSON artifacts from the 30-seed main-method package, QAOA/QUBO ablation, continuation extension, direct collocation, independent-midpoint Hermite-Simpson baseline, independent-HS all-configured headroom, independent-HS branch-control replay, independent-HS bicircular phase-sweep stress, independent-HS cached-Horizons solar-tidal replay, independent-HS cached-Horizons point-mass retuning, independent-HS multi-epoch cached-Horizons point-mass retuning, independent-HS SPICE-derived ephemeris replay, independent-HS CasADi/IPOPT bridge, tail-coast, delayed-recovery, focused tail-coast branch-control replay, Horizons ephemeris contrast, bicircular solar-tidal stress, and bicircular retuned recovery packages | `data/results/claim_evidence_ledger/claim_evidence_ledger.csv`, `data/results/claim_evidence_ledger/claim_evidence_ledger_metadata.json`, `data/results/claim_evidence_ledger/tail_coast_threshold_audit.csv`, `data/results/claim_evidence_ledger/tail_coast_branch_audit.csv`, `tables/claim_evidence_ledger/*` | Short deterministic postprocessor over recorded artifacts; no trajectory optimization is launched by the ledger itself. The current snapshot emits the independent-HS all-configured headroom row, the normalized CR3BP branch-control replay row, the positive simple bicircular phase-sweep stress row, the positive cached-Horizons-derived solar-tidal replay row, the cached-Horizons point-mass retuning row, the multi-epoch cached-Horizons point-mass retuning row, the SPICE-derived ephemeris replay row, the scoped CasADi/IPOPT bridge row, the tail-coast accepted-control replay row, the Horizons force-model contrast row, the negative bicircular solar-tidal stress row, and the completed negative bicircular retuned recovery row because those packages exist. |
| Evidence synthesis replay | `py -3.11 scripts\run_evidence_synthesis.py` | Recorded CSV/JSON artifacts from threshold sensitivity, continuation extension, direct collocation, independent-midpoint Hermite-Simpson baseline, independent-HS all-configured headroom, independent-HS branch-control replay, independent-HS bicircular phase-sweep stress, independent-HS cached-Horizons solar-tidal replay, independent-HS cached-Horizons point-mass retuning, independent-HS multi-epoch cached-Horizons point-mass retuning, independent-HS SPICE-derived ephemeris replay, independent-HS CasADi/IPOPT bridge, and tail-coast packages | `data/results/evidence_synthesis/evidence_synthesis.csv`, `data/results/evidence_synthesis/evidence_synthesis_metadata.json`, `tables/evidence_synthesis/evidence_synthesis_table.tex`, `tables/evidence_synthesis/practitioner_lessons_table.tex` | Short deterministic postprocessor; no trajectory optimization is rerun. |
...
| Independent-HS cached-Horizons solar-tidal replay | `py -3.11 scripts\run_independent_hs_horizons_solar_tidal_replay.py` | Committed cache `data/cache/horizons/independent_hs_phase_shift_2026jan01_vectors.json`, `configs/independent_hs_all_configured_headroom.yaml`, `data/source_states.json`, independent-HS all-configured CSV, replay-ready branch manifests, nominal sidecars, and full-length endpoint-plus-midpoint branch-control sidecars | `data/results/independent_hs_horizons_solar_tidal_replay/independent_hs_horizons_solar_tidal_replay.csv`, `data/results/independent_hs_horizons_solar_tidal_replay/independent_hs_horizons_solar_tidal_replay_metadata.json`, `tables/independent_hs_horizons_solar_tidal_replay/independent_hs_horizons_solar_tidal_replay_table.tex` | Short deterministic postprocessor; default path is offline. It validates cache compatibility for the representative 2026-Jan-01 epoch, transfer time `0.5`, 8-segment grid, canonical time unit `375190.259 s`, and `384400 km/LU`, then repropagates persisted controls with a simplified solar tide from interpolated cached JPL Horizons Sun geometry. For the converged polish row: nominal `0.018363195236986728`, branch worst `0.07422350563850917`, branch pass `8/8`, CR3BP replay delta `0.0`, Sun LU range `382.6857920288508--382.693044178952`, cache SHA-256 `13fe699371ad67bf1616d38b7afd316bbff72811bbc0f8337cff51d6333897b2`. Not SPICE/high-fidelity/flight validation, broad production-solver validation/parity, fuel optimality, or quantum evidence. Use `--refresh-cache` only when intentionally regenerating the JPL Horizons cache. |
| Independent-HS cached-Horizons point-mass retuning | `py -3.11 scripts\run_independent_hs_horizons_point_mass_retuning.py` | Committed cache `data/cache/horizons/independent_hs_phase_shift_2026jan01_vectors.json`, `configs/independent_hs_all_configured_headroom.yaml`, `data/source_states.json`, independent-HS all-configured CSV, replay-ready branch manifests, nominal sidecars, and full-length endpoint-plus-midpoint branch-control sidecars | `data/results/independent_hs_horizons_point_mass_retuning/independent_hs_horizons_point_mass_retuning.csv`, `data/results/independent_hs_horizons_point_mass_retuning/independent_hs_horizons_point_mass_retuning_metadata.json`, `data/results/independent_hs_horizons_point_mass_retuning/controls/*`, `tables/independent_hs_horizons_point_mass_retuning/independent_hs_horizons_point_mass_retuning_table.tex` | Retuning postprocessor; default path is offline and validates the same representative 2026-Jan-01 cache SHA-256 `13fe699371ad67bf1616d38b7afd316bbff72811bbc0f8337cff51d6333897b2`. Persisted controls fail direct Earth/Moon/Sun point-mass replay: nominal `0.3812580376880591`, branch worst `0.3797450961017463`, branch pass `0/8`. Independent retuning restores feasibility: nominal `0.02143944130524006`, branch worst `0.02473065115224942`, branch pass `8/8`, SciPy success `9/9`, total `nfev=71`. Not SPICE/full high-fidelity/flight validation, broad production-solver validation/parity, fuel optimality, DOI evidence, or quantum evidence. |
| Independent-HS multi-epoch cached-Horizons point-mass retuning | `py -3.11 scripts\run_independent_hs_horizons_multi_epoch_point_mass_retuning.py` | Committed caches `data/cache/horizons/independent_hs_phase_shift_2026jan01_vectors.json`, `data/cache/horizons/independent_hs_phase_shift_2026apr01_vectors.json`, `data/cache/horizons/independent_hs_phase_shift_2026jul01_vectors.json`, and `data/cache/horizons/independent_hs_phase_shift_2026oct01_vectors.json`; same config, source states, independent-HS all-configured CSV, manifests, and endpoint-plus-midpoint sidecars as the single-epoch retuning package | `data/results/independent_hs_horizons_multi_epoch_point_mass_retuning/independent_hs_horizons_multi_epoch_point_mass_retuning.csv`, `data/results/independent_hs_horizons_multi_epoch_point_mass_retuning/independent_hs_horizons_multi_epoch_point_mass_retuning_metadata.json`, `data/results/independent_hs_horizons_multi_epoch_point_mass_retuning/epochs/*`, `tables/independent_hs_horizons_multi_epoch_point_mass_retuning/independent_hs_horizons_multi_epoch_point_mass_retuning_table.tex` | Retuning wrapper; default path is offline from committed caches. Aggregates 4 epochs, 36 rows, 32 branch rows. Nominal direct replay fails in 4/4 epochs; direct branch pass count is `18/32` overall, including `8/8` in July. Worst direct nominal/branch errors are `0.3812580376880591`/`0.3797450961017463`. Retuning restores feasibility for all rows: worst retuned nominal/branch errors are `0.02143944130524006`/`0.02473065115224942`, branch pass `32/32`, SciPy success `36/36`, total `nfev=197`. Not SPICE/full high-fidelity/flight validation, broad production-solver validation/parity, fuel optimality, DOI evidence, or quantum evidence. |
| Independent-HS SPICE-derived ephemeris replay | `py -3.11 scripts\run_independent_hs_spice_ephemeris_replay.py`; add `--refresh-spice-cache` only to regenerate compact vector caches from NAIF | Committed compact caches `data/cache/spice/independent_hs_phase_shift_2026*_spice_vectors.json`; source multi-epoch Horizons-retuned controls under `data/results/independent_hs_horizons_multi_epoch_point_mass_retuning/epochs/`; `configs/independent_hs_all_configured_headroom.yaml`, `data/source_states.json`. Refresh downloads `naif0012.tls`, `de442s.bsp`, and `gm_de440.tpc` transiently under ignored `data/cache/spice/kernels/`. | `data/results/independent_hs_spice_ephemeris_replay/independent_hs_spice_ephemeris_replay.csv`, `data/results/independent_hs_spice_ephemeris_replay/independent_hs_spice_ephemeris_replay_metadata.json`, `data/cache/spice/independent_hs_phase_shift_2026*_spice_vectors.json`, `tables/independent_hs_spice_ephemeris_replay/independent_hs_spice_ephemeris_replay_table.tex` | No-retune replay postprocessor; default path is offline and does not import SpiceyPy. Refresh requires `spiceypy==8.1.2`/CSPICE `CSPICE_N0067` and records kernel SHA-256 values. Replays 4 nominal and 32 branch Horizons-retuned controls under SPICE-derived Moon/Sun J2000 geometric vectors with branch pass `32/32`, worst nominal/branch `0.021439441253166033`/`0.024730650824609506`, and max delta from the Horizons-retuned replay `3.2763991519857427e-10`. This is SPICE-derived vector replay under the same point-mass stress model, not full high-fidelity/flight validation, broad production-solver validation/parity, fuel optimality, DOI evidence, or quantum evidence. |
| Independent-HS CasADi/IPOPT bridge check | `py -3.11 scripts\run_independent_hs_casadi_ipopt_bridge.py` | `configs/independent_hs_all_configured_headroom.yaml`, `data/source_states.json`, independent-HS all-configured CSV, replay-ready polish manifest, nominal sidecar, and eight branch sidecars for `ihs_all_single_p04_amax02_polish_from_p04`; `casadi==3.7.2` | `data/results/independent_hs_casadi_ipopt_bridge/independent_hs_casadi_ipopt_bridge.csv`, `data/results/independent_hs_casadi_ipopt_bridge/independent_hs_casadi_ipopt_bridge_metadata.json`, `data/results/independent_hs_casadi_ipopt_bridge/controls/*`, `tables/independent_hs_casadi_ipopt_bridge/independent_hs_casadi_ipopt_bridge_table.tex` | Scoped CasADi/IPOPT direct-shooting refinement under the original normalized CR3BP target/scales/branch recovery semantics. Branch prefixes are fixed to refined nominal masked controls and only post-recovery active branch controls are IPOPT variables. Source replay nominal/branch-worst `0.011333095366088189`/`0.07792080291839382`; IPOPT refined nominal/branch-worst `0.009138565365046585`/`0.015534969964216154`; IPOPT success `9/9`; bridge pass `9/9`; max prefix delta `0.0`; max bound violation `0.0`. Scoped CasADi/IPOPT mature NLP backend bridge check only, not production mission design, high-fidelity/flight validation, global/fuel optimality, DOI evidence, or quantum evidence. |
...
| Online threshold feasible-subspace search | `py -3.11 scripts\run_online_quantum_search.py` | Recorded branch-refined cardinality-ablation costs from `data/results/phase_shift_cardinality_ablation/cardinality_refinements.csv`; configured phase-shift feedback-propagation setup in `configs/q1_phase_shift_cardinality.yaml`; `src/qlt/online_quantum_search.py` | `data/results/online_quantum_search/summary.csv`, `data/results/online_quantum_search/summary.json`, `data/results/online_quantum_search/raw_results.csv`, `data/results/online_quantum_search/paired_comparisons.csv`, `data/results/online_quantum_search/paired_comparisons.json`, `data/results/online_quantum_search/metadata.json`, `data/results/online_quantum_search/*_subspace_costs.csv`, `tables/online_quantum_search/online_quantum_search_table.tex`, `tables/online_quantum_search/online_quantum_search_statistics_table.tex`, `figures/online_quantum_search/online_quantum_search_success.*` | Short deterministic simulation/postprocessor over finite feasible subspaces and feedback-propagation screening costs. The headline online protocol uses current-incumbent threshold comparisons, not a known top-k oracle, and consumes the requested total-oracle budget when feasible by capping Grover iterations to the remaining budget. Deployable matched-budget classical baselines are uniform random, finite-subspace simulated annealing, cross-entropy, and genetic search over the same precomputed subspace cost vectors; the paired comparison artifacts report online-minus-baseline success deltas, bootstrap intervals for mean best-cost differences, and exact sign tests. Recorded branch-refined evidence is limited to `one_coast_k11_full` and `k10_full`; `feedback_N16_k12` and `feedback_N20_k16` are larger CR3BP feedback-screening diagnostics only, not branch-refined trajectory evidence. The surrogate-ranked row is an optimistic ceiling/diagnostic, especially for feedback-screening rows, not a fair deployable baseline or quantum-advantage claim. |
| Online large-case branch-refinement diagnostic | `py -3.11 scripts\run_online_branch_refinement.py` | `configs/q1_phase_shift_cardinality.yaml`, `data/results/online_quantum_search/feedback_N16_k12_subspace_costs.csv`, `data/results/online_quantum_search/feedback_N20_k16_subspace_costs.csv`, `src/qlt/refinement.py` | `data/results/online_branch_refinement/online_branch_refinement.csv`, `data/results/online_branch_refinement/online_branch_refinement_metadata.json`, `tables/online_branch_refinement/online_branch_refinement_table.tex` | Bounded diagnostic that refines only the top feedback-screening schedule per default large case under the configured branch-recovery cap and no multistart expansion. Current rows are negative/partial evidence: `feedback_N16_k12` has branch nominal/selected/all errors `0.13419462945209976`/`0.15380252690824966`/`0.15774161063697187`, optimizer success `False`, and `nfev=35`; `feedback_N20_k16` has `0.11540517588258632`/`0.13157473137165748`/`0.13462092708453266`, optimizer success `True`, and `nfev=28`. Both fail the `0.09` nominal threshold while selected/all robust diagnostics are below `0.17`; large online rows therefore remain feedback-screening evidence until stronger branch-refined feasible seeds are demonstrated. |
| Structured Grover diagnostic sweep | `py -3.11 scripts\run_structured_grover.py`; for the second primary diagnostic, rerun with `--group k10_full --results-dir data/results/structured_grover_k10_full --figures-dir figures/structured_grover_k10_full --tables-dir tables/structured_grover_k10_full` | Recorded branch-refined cardinality-ablation subspaces, `src/qlt/structured_grover.py`, and exact finite-subspace statevector amplification diagnostics | `data/results/structured_grover_one_coast/summary.csv`, `data/results/structured_grover_one_coast/summary.json`, `data/results/structured_grover_one_coast/metadata.json`, `tables/structured_grover_one_coast/*`; matching `data/results/structured_grover_k10_full/*` summary/metadata and `tables/structured_grover_k10_full/*` | Short exact diagnostic sweep. It retains the earlier threshold/top-k Grover-style experiments as controlled statevector diagnostics over recorded costs, including query-scaling, comparison, and selection-bias audit tables. It is not the headline online protocol, does not use hardware, and should not be read as a deployable oracle, quantum advantage, continuous-refinement, or high-fidelity validation claim. |
...
- Reviewer-facing claim evidence ledger:
  ...
  solar-tidal replay row, the independent-HS cached-Horizons point-mass
  retuning row, the independent-HS multi-epoch cached-Horizons point-mass
  retuning row, the independent-HS SPICE-derived ephemeris replay row, the
  independent-HS CasADi/IPOPT bridge row, the
  focused tail-coast branch-control replay row, the Horizons
  force-model contrast row, the negative bicircular solar-tidal stress row, and
  the completed negative bicircular retuned recovery row because their real
  artifacts exist.
- Cross-backend evidence synthesis and practitioner lessons:
  ...
  row, independent-HS cached-Horizons point-mass retuning row, independent-HS
  multi-epoch cached-Horizons point-mass retuning row, and independent-HS
  SPICE-derived ephemeris replay row, and independent-HS CasADi/IPOPT bridge
  row when their real
  CSV and metadata exist.
...
- Independent-HS branch-control replay:
  ...
  high-fidelity validation, broad production-solver validation/parity, fuel optimality, or
  quantum advantage.
...
- Independent-HS cached-Horizons-derived solar-tidal replay:
  ...
  broad production-solver validation/parity, flight-ready evidence, fuel optimality, or quantum
  evidence.
- Independent-HS cached-Horizons Earth/Moon/Sun point-mass retuning:
  `data/cache/horizons/independent_hs_phase_shift_2026jan01_vectors.json`,
  ...
  retuned control sidecars under
  ...
  The default path is offline from the same committed representative
  2026-Jan-01 JPL Horizons Moon/Sun vector cache. It propagates geocentric
  Earth central gravity plus indirect Moon/Sun point-mass terms and retunes
  endpoint-plus-midpoint controls independently for the nominal row and each
  branch, with branch outage segments fixed inactive. Persisted controls fail
  direct point-mass replay: nominal error `0.3812580376880591`, branch worst
  `0.3797450961017463`, branch pass `0/8`. Retuning restores the configured
  screen: nominal error `0.02143944130524006`, branch worst
  `0.02473065115224942`, branch pass `8/8`, SciPy success `9/9`, total
  `nfev=71`. This is retuning evidence under a cached-Horizons point-mass
  stress model only; it is not SPICE/full high-fidelity/flight validation,
  broad production-solver validation/parity, fuel optimality, DOI evidence, or quantum evidence.
- Independent-HS multi-epoch cached-Horizons Earth/Moon/Sun point-mass
  retuning:
  `data/cache/horizons/independent_hs_phase_shift_2026jan01_vectors.json`,
  ...
  per-epoch retuned control sidecars under
  ...
  The default path is offline from the four committed representative 2026
  caches. It aggregates 4 epochs, 36 rows, and 32 branch rows. Nominal direct
  replay fails in 4/4 epochs; direct branch pass count is `18/32` overall,
  including `8/8` in July. Worst direct nominal/branch errors are
  `0.3812580376880591`/`0.3797450961017463`. Retuning restores feasibility for
  all rows: worst retuned nominal/branch errors are
  `0.02143944130524006`/`0.02473065115224942`, branch pass `32/32`, SciPy
  success `36/36`, and total `nfev=197`. This is a stronger representative-
  epoch point-mass stress/retuning set, not SPICE/full high-fidelity/flight
  validation, broad production-solver validation/parity, fuel optimality, DOI evidence, or
  quantum evidence.
- Independent-HS SPICE-derived ephemeris replay:
  ...
  The default path is offline from committed compact JSON vector caches. The
  refresh path downloads NAIF kernels only when `--refresh-spice-cache` is
  supplied, records `naif0012.tls`, `de442s.bsp`, `gm_de440.tpc`, kernel
  SHA-256 values, SpiceyPy `8.1.2`, and CSPICE `CSPICE_N0067`, then removes the
  raw kernel binaries unless `--keep-kernels` is explicitly used. The replay
  reuses the 36 controls already retuned by the multi-epoch cached-Horizons
  point-mass package and performs no optimization or retuning. All 4 nominal
  and all 32 branch rows pass with worst nominal/branch errors
  `0.021439441253166033`/`0.024730650824609506`; the max absolute delta from
  the Horizons-retuned replay is `3.2763991519857427e-10`. This is
  SPICE-derived ephemeris replay under the same point-mass stress model, not
  full high-fidelity/flight validation, broad production-solver validation/parity, fuel
  optimality, DOI evidence, or quantum evidence.
- Independent-HS CasADi/IPOPT bridge check:
  ...
  refined control sidecars under
  ...
  The script exports the accepted polish case's endpoint-plus-midpoint controls
  to a CasADi/IPOPT direct-shooting NLP under the same normalized CR3BP target,
  scales, thresholds, and branch recovery semantics. Branch prefixes are fixed
  to refined nominal masked controls and only post-recovery active branch
  controls are IPOPT variables. Source replay nominal/branch-worst
  errors are `0.011333095366088189`/`0.07792080291839382`; IPOPT refined
  nominal/branch-worst errors are `0.009138565365046585`/`0.015534969964216154`,
  with IPOPT success `9/9`, bridge pass `9/9`, max prefix delta `0.0`, and max control-bound violation
  `0.0`. This is a scoped CasADi/IPOPT mature NLP backend bridge check, not production
  mission design, high-fidelity/flight validation, global/fuel optimality, DOI
...
- Focused tail-coast accepted branch-control replay:
  ...
  high-fidelity validation, fuel-optimal analysis, broad production-solver validation/parity
...
- Bicircular solar-tidal stress replay:
  ...
  ephemeris validation, broad production-solver validation/parity, fuel optimality, or
...
- Bicircular tail-coast retuned recovery:
  ...
  It is not SPICE/high-fidelity/flight validation, broad production-solver validation/parity,
  fuel optimality, quantum, QUBO, or QAOA evidence.
- Online threshold feasible-subspace search:
  ...
  `tables/online_quantum_search/online_quantum_search_statistics_table.tex`, and
  `figures/online_quantum_search/online_quantum_search_success.pdf`. The
  branch-refined tier covers only the recorded `one_coast_k11_full` and
  `k10_full` subspaces. The feedback-screening rows
  `feedback_N16_k12` and `feedback_N20_k16` use CR3BP feedback-propagation
  screening objectives and terminal-error summaries only; they are not
  branch-refined trajectory evidence. The ranked screening row is an
  optimistic ceiling/diagnostic for near-perfect classical ranking information,
  not a fair deployable baseline claim or evidence of quantum advantage.
